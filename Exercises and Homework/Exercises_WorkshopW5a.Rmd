...
```{bash, eval=F}
cp -r /u/scratch/a/abthatt/QCB-W5a-Nov2021/Data/ ~/QCB-W5a-Nov2021/Data/
```
...
First, we have to download some reference data. We're using the human GENCODE Release 38 files. Change your directory to `~/QCB-W5a-Nov2021/Data/`. Run the following commands to download a GTF and a FASTA file for some annotations:

```{bash, eval=F}
wget ftp.ebi.ac.uk/pub/databases/gencode/Gencode_human/release_38/gencode.v38.basic.annotation.gtf.gz
zcat gencode.v38.basic.annotation.gtf.gz | grep -w chr18 > gencode.v38.basic_chr18.gtf

wget ftp.ebi.ac.uk/pub/databases/gencode/Gencode_human/release_38/gencode.v38.pc_transcripts.fa.gz

module load samtools
gunzip gencode.v38.pc_transcripts.fa.gz
grep ">" gencode.v38.pc_transcripts.fa | grep "|18|" | awk 'BEGIN {FS=">"} {print $2}' > selection_18
samtools faidx gencode.v38.pc_transcripts.fa `cat selection_18` > gencode.v38.pc_chr18.fa
```

This will take a little bit of time to download and run. Let's discuss what exactly we're doing in this chunk of code.

Now, we're ready to run STAR.
...
```{bash, eval=F}
fasta_file=../QCB-W5a-Nov2021/Data/gencode.v38.pc_chr18.fa
gtf_file=../QCB-W5a-Nov2021/Data/gencode.v38.basic_chr18.gtf
...
-   To align the cleaned reads of `KD`, we can run the following commands:
    ...
    star_dc_dir=./star_database/
    sample=KD1

    STAR \
    --genomeDir $star_db_dir \
    --readFilesIn $sample.trimmo.fastq.gz \
...
gtf_file=gencode.v38.basic_chr18.gtf
...
1HTSeq1 generates a separate table of per-gene read counts for each sample. For statistical analyses, it is more practical to merge all these tables into one large table comprising the counts at all genes and for all samples.
